...
def parse_args():
    ...
    parser.add_argument('-t', type=int, nargs=2, required=True, help='Target domain info: S, ft.')
    parser.add_argument('-l', type=float, default=5e-3, help='Learning rate.')
...
def test(model, dataloader):
    ...
    with torch.no_grad():
        for data, target in dataloader:
            ...
            output = model(data)
            energys += -(output[1].logsumexp(1).mean())
            logits = F.log_softmax(output[1], dim=-1)
            pred = torch.argmax(logits, dim=-1, keepdim=True)
            correct += pred.eq(target.view_as(pred)).sum().item()
        acc = (correct / len(dataloader.dataset)) * 100
        energy_mean = energys / len(dataloader)
    return correct, acc, energy_mean


def main():
    # dataloader
    x_train, x_valid, x_test, y_train, y_valid, y_test = get_data(config.dataset_path, config.target['s'],
                                                                  config.target['ft'])
    ...
    ckpt = torch.load(f"{config.root_dir}/train/s{config.source['s']}_{config.source['ft']}ft/ckpt.pth")
    ...
    test_correct, test_acc, energy_mean = test(model, dataloader)
    logger.info(f"Testing finished! Energy : {energy_mean:.4f} | ACC : {test_correct}/{len(dataloader.dataset)} ({test_acc:.3f}%) *****\n")
    logger.info("-" * 100 + '\n\n')
