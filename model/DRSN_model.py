...
class RSNet(nn.Module):
    ...
    def __init__(self, c_in, c_out, block, num_block):
        super().__init__()
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(self.c_in, 64, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True))
        ...
        self.fc = nn.Linear(512 * block.expansion, self.c_out)
...
def DRSN_CS18(c_in, c_out):
    return RSNet(c_in, c_out, BasicBlock, [2, 2, 2, 2])


def DRSN_CS34(c_in, c_out):
    return RSNet(c_in, c_out, BasicBlock, [3, 4, 6, 3])


if __name__ == '__main__':
    x = torch.rand((3, 2, 6000), dtype=torch.float32).cuda()
    model = DRSN_CS34(2, 16).cuda()
    y = model(x)
