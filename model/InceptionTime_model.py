...
class InceptionModule(nn.Module):
    def __init__(self, ni, nf, ks=40, bottleneck=True):
        ...
        self.convs = nn.ModuleList([Conv1d_new_padding(nf if bottleneck else ni, nf, k, bias=False) for k in ks])
    ...
    def __init__(self, ni, nf=32, residual=True, depth=6, **kwargs):
        ...
        for d in range(depth):
            ...
            if self.residual and d % 3 == 2:
                ...
                self.shortcut.append(nn.BatchNorm1d(n_in) if n_in == n_out else ConvBlock(n_in, n_out, 1, act=None))
        self.add = Add()
        self.act = nn.ReLU()
