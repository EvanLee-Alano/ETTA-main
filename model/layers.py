...
class SeparableConv1d(nn.Module):
    def __init__(self, ni, nf, ks, stride=1, padding='same', dilation=1, bias=True, bias_std=0.01):
        ...
        self.depthwise_conv = nn.Conv1d(ni, ni, ks, stride=stride, padding=padding, dilation=dilation, groups=ni,
                                        bias=bias)
        self.batchnorm1 = nn.BatchNorm1d(num_features=ni)
        self.pointwise_conv = nn.Conv1d(ni, nf, 1, stride=1, padding=0, dilation=1, groups=1, bias=bias)
        self.batchnorm2 = nn.BatchNorm1d(num_features=nf)
        if bias:
            if bias_std != 0:
